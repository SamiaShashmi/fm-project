...
from .loss_func import SinkhornLoss, MSELoss
...
def scNODETrainWithPreTrain(
    ...
    if pretrain_iters > 0:
        ...
        for t in dim_reduction_pbar:
            ...
            # Backward
            dim_reduction_pbar.set_postfix({"Loss": "{:.3f}".format(vae_loss.item())})
...
def scNODEPredict(latent_ode_model, first_tp_data, tps, n_cells):
    ...
    all_pred_data = all_pred_data.detach().cpu().numpy()  # (# cells, # tps, # genes)
