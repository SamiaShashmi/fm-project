...
import sys
# Add the parent directory (scDiffusion) to the path to find guided_diffusion module
...
def train_vae(args, return_model=False):
    ...
    for step in range(args["max_steps"]):
        ...
        # Ensure genes is converted to torch tensor with float32 dtype
        if not isinstance(genes, torch.Tensor):
            genes = torch.tensor(genes, dtype=torch.float32)
        else:
            genes = genes.to(torch.float32)
